  ...
  $('button, input[type="submit"], input[type="button"], input[type="reset"]').each(
    (_: number, btn: any) => {
      ...
      const isNoise = noisePatterns.some(
        (pattern) => id.toLowerCase().includes(pattern) || classStr.toLowerCase().includes(pattern)
      );
      if (isNoise) return;
...
function extractTextElements(
  ...
  tags.forEach((tag) => {
    $(tag).each((_: number, el: any) => {
      ...
      const isNoise = noisePatterns.some(
        (pattern) => idLower.includes(pattern) || classLower.includes(pattern)
      );
