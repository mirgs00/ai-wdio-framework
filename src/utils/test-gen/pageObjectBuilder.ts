...
export async function buildPageObjects(url: string, htmlContent?: string): Promise<void> {
  ...
  if (htmlContent) {
    ...
    for (const [pageName, code] of Object.entries(pageObjectsByType)) {
      const pageObjectPath = path.join(PAGE_OBJECTS_PATH, `generated${capitalize(pageName)}Page.ts`);
  ...
  logger.info(`✅ Successfully generated all page objects for ${url}`, { section: 'PAGE_OBJECT_BUILD' });
}
...
function detectPageType(htmlContent: string, url: string): string {
  ...
  if (content.includes('logged in') || content.includes('logged-in') || url.includes('logged-in') || content.includes('dashboard')) {
    return 'dashboard';
  }
  ...
  if (content.includes('login') || content.includes('sign in') || content.includes('username') || content.includes('password')) {
    return 'login';
  }
...
function generatePageObjectsByType(elements: PageElement[], url: string, pageType: string): { [key: string]: string } {
  ...
  if (pageType === 'login') {
    ...
    filteredElements = elements.filter(el => 
      el.name.includes('error') || 
      el.description.includes('Error')
    );
  }
  
  // Generate page object for the detected type
  const typeUrl = pageType === 'dashboard' 
    ? url.replace(/\/$/, '') + '/logged-in-successfully/'
    : url;
...
async function identifyPageElements(url: string, htmlContent?: string): Promise<PageElement[]> {
  if (htmlContent) {
    try {
      ...
      logger.warn('Failed to analyze DOM:', { section: 'DOM_ANALYSIS', details: { error: error instanceof Error ? error.message : error } });
    }
  }
