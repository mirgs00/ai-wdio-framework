...
export class StepPatternGenerator {
  ...
  private initializePatterns(): void {
    ...
    this.commonPatterns.set('click-button', {
      ...
      examples: ['I click the "back" button', 'I click the "submit" button', 'I click the "login" button'],
  ...
  private stepMatchesExample(step: string, example: string): boolean {
    ...
    for (let i = 0; i < stepWords.length; i++) {
      ...
      if (stepWord.includes(exampleWord) || exampleWord.includes(stepWord) || stepWord === exampleWord) {
        matches++;
      }
    }
  ...
  private generateGenericPattern(stepText: string): string {
    ...
    const allMatches: Array<{start: number, end: number, type: 'quote' | 'number', original: string}> = [];
    
    while ((match = quotedRegex.exec(stepText)) !== null) {
      allMatches.push({start: match.index, end: match.index + match[0].length, type: 'quote', original: match[0]});
    }
    
    while ((match = numberRegex.exec(stepText)) !== null) {
      allMatches.push({start: match.index, end: match.index + match[0].length, type: 'number', original: match[0]});
    }
